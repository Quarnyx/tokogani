<table id="table-data" class="table table-bordered table-striped">
    ...
    <tbody>
        ...
        $no = 1;
        $sql = "SELECT * FROM v_pengeluaran_barang ORDER BY tanggal DESC";
            ...
            <tr>
                ...
                <td>
                    <div class="d-flex flex-wrap align-items-end gap-1">
                        <button id="edit" data-nama="<?= $row['nama_produk'] ?>" data-id="<?= $row['id_barang_keluar'] ?>"
                        ...
                        <button id="delete" data-nama="<?= $row['nama_produk'] ?>" data-id="<?= $row['id_barang_keluar'] ?>"
...
<script>
    $(document).ready(function () {
        ...
        $('#table-data').on('click', '#edit', function () {
            ...
            $.ajax({
                type: 'POST',
                url: 'pages/pengeluaran-barang/edit-pengeluaran-barang.php',
        ...
        $('#table-data').on('click', '#delete', function () {
            ...
            alertify.confirm('Hapus', 'Apakah anda yakin ingin menghapus data ' + nama + '?', function () {
                $.ajax({
                    type: 'POST',
                    url: 'pages/pengeluaran-barang/proses-pengeluaran-barang.php?aksi=hapus-pengeluaran-barang',
                    data: 'id=' + id,
                    success: function (data) {
                        if (data == "ok") {
                            loadTable();
