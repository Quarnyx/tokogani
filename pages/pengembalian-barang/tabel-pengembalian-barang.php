            ...
            <tr>
                ...
                <td>
                    <div class="d-flex flex-wrap align-items-end gap-1">
                        <button id="edit" data-nama="<?= $row['nama_produk'] ?>" data-id="<?= $row['id_barang_kembali'] ?>"
...
<script>
    $(document).ready(function () {
        ...
        $('#table-data').on('click', '#edit', function () {
            ...
            $.ajax({
                type: 'POST',
                url: 'pages/pengembalian-barang/edit-pengembalian-barang.php',
        ...
        $('#table-data').on('click', '#delete', function () {
            ...
            alertify.confirm('Hapus', 'Apakah anda yakin ingin menghapus data ' + nama + '?', function () {
                $.ajax({
                    type: 'POST',
                    url: 'pages/pengembalian-barang/proses-pengembalian-barang.php?aksi=hapus-pengembalian-barang',
                    data: 'id=' + id,
                    success: function (data) {
                        if (data == "ok") {
                            loadTable();
